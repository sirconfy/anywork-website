...
export default function BlogCard({ tag, tagColor, title, date, time, image, desc }: BlogCardProps) {
  return (
    <div className="w-full border border-[#E6E6E6] rounded-2xl overflow-hidden bg-white">
      ...
      {/* Content Section */}
      <div className="p-4">
        <h3 className="text-[18px] font-[600] text-textcolor mb-2">{title}</h3>
        ...
        <div className="flex justify-between items-center my-6">
          ...
          <div className="flex items-center  rounded-full px-4 border border-[#282E3280]">
            ...
            <span  className="text-[#282E3280] font-[400] text-[14px]">{time}</span>
          </div>
        </div>

        <button className="text-sm text-[#026994] text-[14px] font-[400] border border-[#026994] rounded-xl w-full px-4 py-3 flex items-center justify-center gap-2 transition hover:bg-[#026994] hover:text-white">
  <span>Read Article</span>
  <FiArrowRight className="text-lg text-[#026994]" />
</button>
      </div>
    </div>
  );
